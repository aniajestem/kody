...
def kwerenda1(cur):
    cur.execute("""
    SELECT klasa, ocena, przedmiot FROM oceny
    INNER JOIN przedmioty ON przedmioty.id = oceny.id_przedmiot
    INNER JOIN uczniowie ON uczniowie.ide = oceny.id_uczen
    INNER JOIN klasa ON przedmioty.id = oceny.id przedmioty
    GROUP BY srednia DESC
